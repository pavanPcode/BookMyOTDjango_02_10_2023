...
def add_pre_existing_condition_form(request):
    if request.method == 'POST':
        ...
        data = {"inputdata": {"name": name, 'code': code, "description": desc, "type": type,"preexistingconditionid": 0}}
...
def config_post_config_pre_Existing(request):
    if request.method == 'POST':
        ...
        type = request.POST.get('preExConType')
        desc = request.POST.get('preExConDesc')
        print(pid,code,name,desc, type, 'preexistingcoindingtons values')
        data = {"inputdata": {"code": code, "name": name, "description": desc, 'type': type,"preexistingconditionid":pid}}
